...
fn engine(server: &MockServer, meta: Arc<dyn MetaStore>, files: Arc<dyn FileStore>) -> SyncEngine {
...
async fn incremental_pull_uses_stored_checkpoint() {
  ...
  Mock::given(method("GET"))
    ...
    .and(query_param_is_missing("since"))
    .respond_with(
      ResponseTemplate::new(200).set_body_json(changes_body("cp-1", &[("a.txt", false, "1-a")])),
    )
    .mount(&server)
    .await;
  Mock::given(method("GET"))
    ...
    .respond_with(
      ResponseTemplate::new(200).set_body_json(changes_body("cp-2", &[("b.txt", false, "1-b")])),
    )
    .mount(&server)
    .await;
  Mock::given(method("GET"))
