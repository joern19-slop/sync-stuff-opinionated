...
mod tests {
    ...
    async fn mem_meta_store_roundtrips() {
        ...
        assert_eq!(MetaStore::list_keys(&s, "file/").await.unwrap(), Vec::<String>::new());
        MetaStore::put(&s, "file/x", b"m".to_vec()).await.unwrap();
        assert_eq!(MetaStore::list_keys(&s, "file/").await.unwrap(), vec!["file/x"]);
    ...
    async fn mem_file_store_roundtrips() {
        ...
        FileStore::put(&s, "a.txt", b"hello".to_vec(), 1).await.unwrap();
        assert_eq!(FileStore::get(&s, "a.txt").await.unwrap(), Some(b"hello".to_vec()));
